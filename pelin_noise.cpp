...
double CPerlinNoise::Noise(int x, int y) {
	int n = x + y * 57;
	n = (n << 13) ^ n;
	return (1.0 - ((n * (n * n * 15731 + 789221) + 1376312589) & 0x7fffffff) / 1073741824.0);
}

double CPerlinNoise::SmoothNoise(double x, double y) {
	double corners = (Noise(x - 1, y - 1) + Noise(x + 1, y - 1) + Noise(x - 1, y + 1) + Noise(x + 1, y + 1)) / 16;
	double sides = (Noise(x - 1, y) + Noise(x + 1, y) + Noise(x, y - 1) + Noise(x, y + 1)) / 8;
	double center = Noise(x, y) / 4;
	return corners + sides + center;
}
double CPerlinNoise::Interpolate(double a, double b, double x) {
...
double CPerlinNoise::PerlinNoise_2D(double x, double y) {
	double total = 0;
...
CPerlinNoise::CPerlinNoise(int seed)
{
	m_seed = seed;
	persistence = 0.4;
	Number_Of_Octaves = 5;
